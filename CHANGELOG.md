...
- **2026-08-24 dependency re-audit — clean, no new advisories.**
  ...
  the incoming pins and again after the `uvicorn 0.52.4` refresh below.
  ...
  2026-08-17 re-audit — coverage of the Starlette "BadHost" bypass
  ...
  prune; `.gitignore` reviewed, already comprehensive. No open pull requests
  or issues on the repository. Nothing to patch; no advisory ≥ 7.5 CVSS (or
  any severity) observed against this stack this run. (Note: the Dependabot
  alerts API was not reachable from this run's environment; coverage came
  from `pip-audit` — PyPI advisory DB, which mirrors OSV/GHSA — plus the
...
### Dependencies
- **2026-08-24: `uvicorn 0.52.3 → 0.52.4`.** Maintenance bump to current
  stable (single bugfix release: removes a duplicate `Date` header from
  accepted WebSocket handshakes under the `websockets-sansio` implementation;
  no security content; both old and new pins audit clean). Smoke-tested on
  the bumped set: app imports and starts, `/login` serves 200,
